...
import type { Storage } from '@/infraestructure/storage';
import { STORAGE } from '@/infraestructure/storage';
import { AgentsLoader } from './loader/agents-loader';
import { AgentDefinition } from '@/definitions/models/agent-definition/agent-definition';
import { AgentRole } from '@/definitions/models/agent-definition/agent-definition';
...
export class AgentDefinitionService {
   ...
   * and storage backend.
   *
   * @param agentsLoader - The loader responsible for reading bundled agent definitions from disk.
  ...
  constructor(
    @Inject(AgentsLoader)
    private readonly agentsLoader: AgentsLoader,
   ...
   */
  async find(id: string): Promise<AgentDefinition> {
    let agent: AgentDefinition | null = null;

    try {
      agent = await this.storage.read<AgentDefinition>(id);
    } catch (error) {
      throw new AgentServiceError(`Failed to retrieve agent ${id}`, {
        cause: error,
      });
    }
  ...
  async getMainAgent(): Promise<AgentDefinition> {
    ...
    try {
      ...
      if (!mainAgentId) throw new MainAgentNotFoundError();
    ...
    if (!agent) {
      throw new MainAgentNotFoundError();
    }
  ...
  async registerBundledAgents(): Promise<void> {
    const agents = await this.agentsLoader.loadAgentsFromRootDirectory(
