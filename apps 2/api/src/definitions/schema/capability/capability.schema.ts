...
/**
 * Defines the schema used to validate a capability configuration file.
 *
 * The `capabilitySchema` describes identity and versioning, classification
 * (integration versus domain capability), exposed tools and actions, execution
 * limits, safety and constraint knobs, authentication expectations, and typed
 * configuration entries for runtime wiring.
 *
 * Integration capabilities typically connect to external systems or APIs,
 * while domain capabilities model reusable behavior within a problem space;
 * both are attachable building blocks that agents reference by capability id.
 */
