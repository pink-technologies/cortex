...
import { AgentsRepository } from '@/agents/repositories/index';
import {
  AgentDto,
  CreateAgentParametersDto,
  UpdateAgentParametersDto,
} from '@/agents/dto/index';
import {
  AgentNotFoundError,
  AgentRequiredIdError,
} from '@/agents/service/error/agents.error';
...
export class AgentsService {
  ...
  async update(
    id: string,
    parameters: UpdateAgentParametersDto,
  ): Promise<AgentDto> {
    if (!id) throw new AgentRequiredIdError();
    ...
    const updatedAgent = await this.agentsRepository.update(
      id,
      parameters.name,
      parameters.description,
    );
