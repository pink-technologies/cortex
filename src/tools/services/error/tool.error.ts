...
/**
 * Error thrown when a tool operation that requires a tool id
 * is invoked with an empty or whitespace-only slug.
 *
 * This typically happens when the caller passes an empty string
 * or only spaces to a service method that expects an id (e.g. isToolRegistered).
 */
export class ToolRequiredIdError extends ToolServiceError {
    // MARK: - Properties

    /**
     * A machine-readable error code identifying missing tool id errors.
     */
    readonly code = 'TOOL_REQUIRED_ID';
}
