...
export function assertRepositoryReviewRuntimeReady(
  ...
  if (configuration.sourceControlConnections.length === 0) {
    throw new Error(
      'CORTEX_SC_CONNECTIONS must include at least one GitHub connection because this Node advertises repository.review.',
    )
  }

  const hasGitHub = configuration.sourceControlConnections.some((connection) => {
    return connection.provider === 'github'
  })

  if (!hasGitHub) {
    throw new Error(
      'CORTEX_SC_CONNECTIONS must include a provider "github" connection because this Node advertises repository.review.',
    )
  }
}
