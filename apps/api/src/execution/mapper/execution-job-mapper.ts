...
import { ExecutionJob, ExecutionJobPolicy, ExecutionJobRequirements, ExecutionJobSource } from '../models'
import { ExecutionJob as ExecutionJobRecord, ExecutionJobStatus as DatabaseExecutionJobStatus } from '@/infraestructure/database'
...
export class ExecutionJobMapper {
  ...
  private static mapStatus(status: DatabaseExecutionJobStatus): ExecutionJobStatus {
    switch (status) {
      ...
      case DatabaseExecutionJobStatus.QUEUED:
        return ExecutionJobStatus.QUEUED

      case DatabaseExecutionJobStatus.RUNNING:
