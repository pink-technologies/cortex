...
export interface TrelloCreateCardRequest {
  /**
   * Optional description placed on the new card.
   */
  readonly description?: string;

  /**
   * List id that receives the card.
   */
 ...
 *
 * Creates cards on a list. Transport and auth are provided by the injected
 * {@link TrelloClient}. Wire payloads are mapped to {@link TrelloCard} via
 * {@link TrelloCard.from}.
 */
export class TrelloCardResource {
   ...
   * @param signal - Optional abort signal for the in-flight request.
