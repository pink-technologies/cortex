...
/**
 * Thrown when a tool's output fails validation against its Zod output schema.
 *
 * This error preserves the originating tool request identity and the Zod
 * validation failure so callers can surface schema issues to diagnostics
 * without losing context.
 */
export class AgentToolOutputValidationError extends AgentRuntimeError {
  // MARK: - Properties

  /**
   * Machine-readable code for tool output validation errors.
   */
  readonly code = 'AGENT_TOOL_OUTPUT_VALIDATION_ERROR'

  /**
   * Name of the tool whose output failed validation.
   */
  readonly toolName: string
  ...
  /**
   * Validation error produced by the tool output schema.
   */
  readonly validationError: ZodError
  ...
  /**
   * Creates an error describing invalid tool output.
   *
   * @param toolName - Name of the requested tool.
   * @param toolUseId - Identifier of the originating tool request.
   * @param validationError - Error produced by the output schema.
   */
  constructor(toolName: string, toolUseId: string, validationError: ZodError) {
    super(`The output for agent tool '${toolName}' is invalid.`, {
      cause: validationError,
    })
...
/**
 * Thrown when a tool requires permissions that are not granted on the current
 * {@link AgentExecutionContext}.
 *
 * This error preserves the originating tool request identity and the missing
 * permission identifiers so callers can surface authorization failures to
 * diagnostics without losing context.
 */
export class AgentToolPermissionDeniedError extends AgentRuntimeError {
  // MARK: - Properties

  /**
   * Machine-readable code for tool permission denial errors.
   */
  readonly code = 'AGENT_TOOL_PERMISSION_DENIED'

  /**
   * Permission identifiers required by the tool that are absent from
   * {@link AgentExecutionContext.permissions}.
   */
  ...
  /**
   * Creates an error describing a tool permission denial.
   *
   * @param toolName - Name of the requested tool.
   * @param toolUseId - Identifier of the originating tool request.
   * @param missingPermissions - Required permissions not granted on the
   *   execution context.
   */
  constructor(toolName: string, toolUseId: string, missingPermissions: readonly string[]) {
    const missing = missingPermissions.join(', ')

    super(
      missing.length > 0
        ? `Permission denied for agent tool '${toolName}': missing ${missing}.`
        : `Permission denied for agent tool '${toolName}'.`,
    )
